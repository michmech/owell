...
// const archival_object_id = "21264";
// const archival_object_id = "22877";
...
if(!error && session) try {
  ...
  if(resp.ok){
    ...
    (json.linked_agents || []).forEach(el => {
      if(el.ref) it.speakers.push(el.ref);
    });
    ...
    (json.ancestors || []).forEach(el => {
      if(el.level && el.level=="series" && el.ref) it.tapes.push(el.ref);
    });
  }
} catch(e){
  error = e;
...
//get the SSSA tapes:
if(!error) for(let i=0; i<it.tapes.length; i++){
  try {
    const resp = await fetch(
      `https://tobarapi.is.ed.ac.uk${it.tapes[i]}`,
      {method: "GET", headers: {"X-ArchivesSpace-Session": session}}  
    );
    if(resp.ok){
      const json = await resp.json();
      // fs.writeFile("./test.json", JSON.stringify(json, null, "  "));
      const tape = {};
      tape.id = it.tapes[i].split("/")[4];
      if(json.title){
        tape.title = json.title;
      }
      if(tape.title) it.tapes[i] = tape;
    }
  } catch(e){
    error = e;
  }
}
it.tapes = it.tapes.filter(x => typeof(x)=="object");
